...
def register_mysql_service(service: FastMCP) -> None:
    ...
    def query_mysql_database(sql_query: str) -> Dict[str, Any]:
        ...
        if not clean_query.startswith(allowed_prefixes):
            return _error_response(message="安全防御限制：本接口仅支持以 SELECT, DESCRIBE, SHOW, EXPLAIN 开头的只读语句。", error_code="MYSQL_PREFIX_ERROR")

        for kw in forbidden_keywords:
            if kw in clean_query:
                return _error_response(message=f"安全防御限制：查询语句中包含禁止的敏感关键字: {str(kw)}", error_code="MYSQL_FORBIDDEN_KEYWORD")
