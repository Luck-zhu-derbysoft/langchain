...
from app.infrastructure.security.auth import Role, TokenPayload, create_access_token, require_roles
...
async def get_config(key: str, current_user:TokenPayload = Depends(require_roles(Role.ADMIN, Role.OPERATOR, Role.AUDITOR))):
...
async def reset_config(key: str,user_id:str="system", current_user:TokenPayload = Depends(require_roles(Role.ADMIN, Role.OPERATOR, Role.AUDITOR))):
    try:
...
async def get_config_change_history(limit: int = 50, current_user:TokenPayload = Depends(require_roles(Role.ADMIN, Role.OPERATOR, Role.AUDITOR))):
