...
def query_local_database(sql_query: str) -> Dict[str, Any]:
    """
    具体的 Skill 执行体：安全地查询本地 SQLite 数据库
    """
    ...
    if not clean_query.startswith("select"):
        return {"status": "error", "message": "安全防御限制：本接口仅支持以 SELECT 开头的查询语句。"}
    ...
    try:
        ...
        return {"status": "error", "message": f"数据库权限或操作异常（可能触发了只读保护）: {str(e)}"}
    except Exception as e:
        return {"status": "error", "message": f"执行 SQL 查询时发生未知错误: {str(e)}"}
    finally:
        if conn:
            conn.close()
...
# 技能路由字典映射
DB_SKILL_MAP = {
    "query_local_database": query_local_database
}
