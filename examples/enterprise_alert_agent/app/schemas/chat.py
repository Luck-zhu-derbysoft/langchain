...
class ChatResponse(BaseModel):
    ...
    model: str
    request_id: str
    intent: str | None = Field(default=None, description="意图识别结果")
    intent_confidence: float | None = Field(default=None, description="意图识别置信度")
    ...
    multi_task_results: dict | None = Field(default=None, description="多任务执行统计结果")
    ...
    active_agent_id: str | None = Field(default=None, description="当前处理请求的主Agent")
    assigned_agent_ids: list[str] = Field(default_factory=list, description="分配的Agent ID列表")
    performance_metrics: dict | None = Field(default=None, description="性能指标")
    # 示例结构:
    # {
    #   "total_time_ms": 1234,
    #   "p50_latency_ms": 100,
